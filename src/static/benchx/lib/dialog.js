		...
		function($rootScope, $modal) {
			...
			return {
				...
				simpleDialog : function(templateUrl, controller, resultFn) {
					return function() {
						return dialog(modalOptions(templateUrl, controller),
								resultFn);
					};
				},

				/**
				 * Opens simple generic dialog presenting title, message (any
				 * html) and provided buttons.
				 */
				messageBox : function(title, message, buttons, resultFn) {
					...
					return dialog(modalOptions(
							"template/messageBox/message.html",
							'MessageBoxController', scope), function(result) {
...
dialogModule.controller('MessageBoxController', [ '$scope', '$modalInstance',
